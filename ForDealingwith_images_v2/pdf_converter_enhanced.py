...
def process_pdf(pdf_path: Path, source_dir: Path, output_dir: Path, embed_images: bool):
...
def process_directory(source_dir: Path, output_dir: Path, embed_images: bool):
    """Process all PDFs in a directory recursively"""
    for root, _, files in os.walk(source_dir):
        for file in files:
            if file.lower().endswith(".pdf"):
                pdf_path = Path(root) / file
                process_pdf(pdf_path, source_dir, output_dir, embed_images)
...
if __name__ == "__main__":
    ...
    process_directory(source_dir, output_dir, embed_images)